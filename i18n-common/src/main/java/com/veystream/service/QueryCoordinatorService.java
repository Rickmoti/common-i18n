...
import com.veystream.dao.GenericEntityDAO; // Import GenericEntityDAO
import com.veystream.search.dto.ExactQueryCondition; // New DTO for exact conditions
import com.veystream.search.dto.PageQuery; // New DTO for pagination
...
public class QueryCoordinatorService {
    ...
    private final EnhancedMultilingualIdFetcher idFetcher;
    private final SearchConfigProvider searchConfigProvider;
    private final GenericEntityDAO genericEntityDAO; // Uncommented
    ...
    @Autowired
    public QueryCoordinatorService(EnhancedMultilingualIdFetcher idFetcher,
                                   SearchConfigProvider searchConfigProvider,
                                   GenericEntityDAO genericEntityDAO) { // Added genericEntityDAO
        this.idFetcher = idFetcher;
        this.searchConfigProvider = searchConfigProvider;
        this.genericEntityDAO = genericEntityDAO; // Uncommented
    }

    // Return type changed to List<Map<String, Object>> to match DAO
    public <T> List<Map<String, Object>> searchEntities(Object criteriaDTO, Class<T> entityClass, PageQuery pageQuery) {
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                if (field.isAnnotationPresent(KeywordSearch.class)) {
                    ...
                    Set<Long> ids = idFetcher.fetchIdsByKeyword(
                            (String) value, entityConfig, Arrays.asList(ann.entityFieldNames()),
                            currentLanguage, defaultLanguage, i18nTypeDb);
                    allMultilingualIdSets.add(ids);
                    log.debug("KeywordSearch on DTO field '{}' for entity fields '{}' with value '{}' found IDs: {}", field.getName(), Arrays.toString(ann.entityFieldNames()), value, ids.size());
                    ...
                    Set<Long> ids = idFetcher.fetchIdsByExactMultilingualField(
                            (String) value, entityConfig, ann.entityFieldName(),
                            currentLanguage, defaultLanguage, i18nTypeDb);
                    allMultilingualIdSets.add(ids);
                     log.debug("MultilingualField on DTO field '{}' for entity field '{}' with value '{}' found IDs: {}", field.getName(), ann.entityFieldName(), value, ids.size());
                    ...
                    log.debug("ExactMatchField on DTO field '{}' for entity field '{}' with value '{}'", field.getName(), ann.entityFieldName(), value);
                }
        ...
        if (multilingualConditionPresent) {
            ...
            if (finalIdsToFilterBy.isEmpty()) {
                log.debug("Intersection of multilingual ID sets is empty. No results for DTO: {}", criteriaDTO.getClass().getSimpleName()); // criteriaDTO itself, not criteriaDTO.getClass().getSimpleName()
                return Collections.emptyList();
            }
            log.debug("Final intersected IDs from multilingual search: {}", finalIdsToFilterBy.size());
        }
        
        // Actual call to GenericEntityDAO
